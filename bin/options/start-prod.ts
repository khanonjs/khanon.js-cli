...
export class StartProd extends OptionBase {
  async execute() {
    console.log(chalk.cyan('Starting Khanon.js project in production environment...\n'))
  ...
  async start() {
    ...
    if (integrityResult.missing.length > 0) {
      ...
      const port = typeof argv.port === 'number' ? String(argv.port) : '3000'

      execSync(`npx rimraf ./${dest}`, { cwd: rootFolder })

      const command = 'node'
      const args = ['node_modules/@khanonjs/engine/builder/start-prod.mjs', '--dest', dest, '--port', port]
