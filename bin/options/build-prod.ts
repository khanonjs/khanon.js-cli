...
export class BuildProd extends OptionBase {
  async execute() {
    console.log(chalk.cyan('Building Khanon.js project in production environment...\n'))
  ...
  async start() {
    ...
    if (integrityResult.missing.length > 0) {
      ...
      const command = 'node'
      const args = ['node_modules/@khanonjs/engine/builder/build-prod.mjs', '--dest', dest]
