...
export class StartDev extends OptionBase {
  async execute() {
    console.log(chalk.cyan('Starting Khanon.js project in development environment...\n'))
  ...
  async start() {
    ...
    if (integrityResult.missing.length > 0) {
      ...
      const command = 'node'
      const args = ['node_modules/@khanonjs/engine/builder/start-dev.mjs', '--dest', dest, '--port', port]
